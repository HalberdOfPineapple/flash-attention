    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if(is_skip){
            ...
            if (!Is_causal && !Is_local && !Is_exact_streaming) {
                ...
                // if (cute::thread0()) { print(idx_row.layout()); print(stride<1>(idx_row)); printf("stride = %d \n", get<1>(stride<1>(idx_row))); }
                // I can't get the stride from idx_row
                flash::apply_mask_streaming</*HasWSLeft=*/Is_exact_streaming>(
                    scores, n_block * kBlockN, binfo.actual_seqlen_k,
                    m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4,
                    binfo.actual_seqlen_q, kNWarps * 16,
...
inline __device__ void compute_block_attn(const Params &params) {
    ...
    if (head_mask_type == 0){
        ...
        flash::compute_block_attn_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_local, Is_even_MN, Is_even_K, Return_softmax, false, false>(params, bidb, bidh, m_block); //false for blocksparse
    }else{
        flash::compute_block_attn_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_local, Is_even_MN, Is_even_K, Return_softmax, true, Is_exact_streaming>(params, bidb, bidh, m_block); // true for streaming
    }
